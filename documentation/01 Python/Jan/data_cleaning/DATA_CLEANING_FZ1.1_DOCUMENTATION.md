# Data Cleaning Documentation - FZ1.1

## 1. Project Overview
**Business Problem and Context:**
- This script processes KBA (Kraftfahrt-Bundesamt) vehicle registration data from sheet FZ1.1
- Data contains vehicle registrations by registration districts (Zulassungsbezirke)
- Key challenges include handling merged cells, inconsistent headers, and data standardization
- Expected output is a cleaned dataset with standardized headers and consistent data format
...
def remove_sum_rows(df, filter_words='zusammen|insgesamt'):
    ...
    for column in df.columns:
        column_mask = ~df[column].fillna('').astype(str).str.contains(
...
def validate_headers():
    """Validate headers across all years."""
...
def process_single_file(file_path: Path) -> pd.DataFrame:
    ...
    try:
        # 1. Read Excel file
        df = pd.read_excel(file_path, sheet_name='FZ1.1')
...
def main():
    ...
    try:
        ...
        for year in YEARS_TO_PROCESS:
            input_file = INPUT_DIR / f'FZ1.1_{year}.xlsx'
            output_file = OUTPUT_DIR / f'FZ1.1_{year}_cleaned.csv'
            ...
            report_file = OUTPUT_DIR / f'FZ1.1_{year}_validation_report.txt'
...
```
Data Validation Report - FZ1.1_2023
==================================
1. Data Completeness
   - Total rows: 15,432
   - Missing values: 0.02%
