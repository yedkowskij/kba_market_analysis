# Data Cleaning Documentation - FZ2.2

## 1. Project Overview
**Business Problem and Context:**
- This script processes KBA (Kraftfahrt-Bundesamt) vehicle registration data from sheet FZ2.2
- Data contains vehicle registrations by manufacturers and model names
- Key challenges include handling manufacturer and model name standardization, merged cells, and data consistency
...
def validate_headers():
    ...
    for year in YEARS_TO_PROCESS:
        ...
        try:
            ...
            # Store and validate header information
```
...
def process_single_file(file_path: Path) -> pd.DataFrame:
    ...
    try:
        # 1. Read Excel file
        df = pd.read_excel(file_path, sheet_name='FZ2.2')
...
def main():
    ...
    try:
        ...
        # Process each year
        for year in YEARS_TO_PROCESS:
            input_file = INPUT_DIR / f'FZ2.2_{year}.xlsx'
            output_file = OUTPUT_DIR / f'FZ2.2_{year}_cleaned.csv'
            ...
            report_file = OUTPUT_DIR / f'FZ2.2_{year}_validation_report.txt'
...
```
Data Validation Report - FZ2.2_2023
==================================
1. Data Completeness
   - Total rows: 15,432
   - Missing values: 0.02%
